...
5060 IF LEFT(CC$,6)<>"UPTIME" THEN 10020 ! IF NOT RUN FROM CCL OR CCL NOT VALID, SKIP
...
5140 IF INSTR(0,CC$,"/HELP")<>0% THEN GOSUB 16000 \ GOTO 32767 ! DISPLAY HELP TEXT AND EXIT
5200 IF OP% AND 8% THEN PRINT "CCL:";CC$
10000 ! MAIN CODE
10020 J%=PEEK(36%)/1000%+2%
10030 J2%=(PEEK(512%)/1000% <> PEEK(36%)/1000%)
10040 J2%=635%+((J%/4%*4%)=J%) IF J2%
10050 T=((PEEK(512%)-PEEK(36%)-J2%)*1440.+PEEK(38%)-PEEK(514%))*60.+60.-(PEEK(516%) AND 255%)
10070 ! &
...
10490 IF OP% AND 8% THEN PRINT "T(Total Seconds),T1(Days),T2(Day_Fraction),T3(Hours),T4(Hours_Fraction),T5(Minutes),T6(Seconds)=";T,T1,T2,T3,T4,T5,T6
12000 IF (OP% AND 4%) THEN PRINT CVT$$(RIGHT(SYS(CHR$(6%)+CHR$(9%)+CHR$(0%)),3%),4%)
...
13800 ! CLEAR CORE COMMON STRING ON EXIT
13810 X$=SYS(CHR$(8%)+"")
13990 GOTO 32767
...
16082 PRINT
16085 PRINT "UPTIME in CCL (UP-TIME) will default to the short (/SHORT) report."
16090 PRINT
16200 RETURN
32767 END
